<?php
// search for movies by chosen director
function searchMoviesByDirector($directorName) {
	include_once ("includes/connectDB.php");
	$db = getDBConnection(); // db connection
	// get movies from db
	$sql = "SELECT director.*, genre.genre_name, 
	...
	WHERE director.director_name = '$directorName' 
	ORDER BY movie.title";
	// query db for director match
	$stmt = $db->prepare($sql);
	$stmt->execute();
	// result
...
function printHtml($m, $rentButton) {
	...
	echo '<p class="l-align-txt"><span class="orange-text">Tagline: </span>'.$m[6].'</p>
		...
		<p class="l-align-txt"><span class="orange-text">Director: </span>'.$m[1].'</p>
		<p class="l-align-txt"><span class="orange-text">Studio: </span>'.$m[3].'</p>
		<p class="l-align-txt"><span class="orange-text">Genre: </span>'.$m[2].'</p>
